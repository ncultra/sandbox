				       ...
				       const char *filename)
{
	...
	if (strlen (filename) != 48) {
		fprintf (stderr,
			 "error: filename must be of form <sha1>.raxlpxs\n");
		return -1;
	}
...
static int get_patch_version (int fd, const char *filename)
{
	...
	if (memcmp (signature, XSPATCH_COOKIE, sizeof (signature)) == 0)
		return 2;
	else if (memcmp (signature, XSPATCH_COOKIE3, sizeof (signature)) ==
		 0)
		return 3;
	return -1;
}
...
static off_t calc_file_size (int fd, const char *filename)
{
	struct stat st;
	if (fstat (fd, &st) < 0) {
...
static int calculate_file_sha1 (int fd, const char *filename,
				unsigned char *hash)
{
	off_t file_size = calc_file_size (fd, filename);
...
static int read_refxen_data (int fd, const char *filename,
			     struct patch *patch)
{
	/* Read Xen version and compile date */
	if (_read (fd, filename, patch->xenversion,
...
static int read_blob_data (int fd, const char *filename,
			   struct patch *patch)
{
	/* Pull the blob out */
...
static int read_reloc_data (int fd, const char *filename,
			    struct patch *patch)
{
...
static int read_check_data (int fd, const char *filename,
			    struct patch *patch)
{
	size_t i;
	...
	for (i = 0; i < patch->numchecks; i++) {
		...
		if (_readu64 (fd, filename, (uint64_t *) & check->hvabs) <
		    0)
			return -1;
...
static int read_func_data (int fd, const char *filename,
			   struct patch *patch)
{
	size_t i;
	...
	patch->funcs =
	    _zalloc (patch->numfuncs * sizeof (patch->funcs[0]));
...
static int read_table_data (int fd, const char *filename,
			    struct patch *patch)
{
	size_t i;
	...
	patch->tables =
	    _zalloc (sizeof (struct table_patch) * patch->numtables);
	for (i = 0; i < patch->numtables; i++) {
		...
		if (_read (fd, filename, table->tablename, tablenamelen) <
		    0)
			return -1;

		if (_readu64 (fd, filename, (uint64_t *) & table->hvabs) <
		    0)
			return -1;
...
static int extract_sha1_from_patch (unsigned char *sha1, size_t sha1len,
...
static int read_tag_data (int fd, const char *filename,
			  struct patch3 *patch)
{
	uint16_t size;
	if (_readu16 (fd, filename, &size) < 0)
...
static int read_deps_data (int fd, const char *filename,
			   struct patch3 *patch)
{
	size_t i;
	if (_readu16 (fd, filename, &patch->numdeps) < 0)
		return -1;
	patch->deps =
	    _zalloc (sizeof (struct dependency) * patch->numdeps);
	for (i = 0; i < patch->numdeps; i++) {
		struct dependency *dep = &patch->deps[i];
		if (_read (fd, filename, &dep->sha1, sizeof (dep->sha1)) <
		    0)
			return -1;
...
static int read_reloc_data3 (int fd, const char *filename,
			     struct patch3 *patch)
{
	size_t i;
	if (_readu16 (fd, filename, &patch->numrelocs3) < 0)
		return -1;
	patch->relocs3 =
	    _zalloc (sizeof (struct reloc3) * patch->numrelocs3);
...
static int read_symbols_data (int fd, const char *filename,
			      struct patch3 *patch)
{
	size_t i;
	if (_readu16 (fd, filename, &patch->numsymbols) < 0)
		return -1;
	patch->symbols =
	    _zalloc (sizeof (struct symbol) * patch->numsymbols);
...
static int _load_patch_file3 (int fd, const char *filename,
			      struct patch3 *patch)
{
