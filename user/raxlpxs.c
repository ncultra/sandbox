...
int _do_lp_buf_op_both (xc_interface_t xch, void *buf, size_t buflen,
			uint64_t op)
{
...
#ifndef sandbox_port
int _do_lp_buf_op (xc_interface_t xch, void *buf, size_t buflen,
		   uint64_t op)
{
#ifdef DECLARE_HYPERCALL_BOUNCE
	DECLARE_HYPERCALL_BOUNCE (buf, buflen,
...
int usage (char *argv0)
{
	...
	fprintf (stderr,
		 "	 info <filename>\tDisplay information about patch\n");
	fprintf (stderr, "	 undo <sha1>\tUn-apply a patch\n");
	fprintf (stderr,
		 "	 listj\t\tList all applied patches (json format)\n");
	fprintf (stderr,
...
void patch_writes (struct patch *patch, struct xenlp_patch_write *writes)
{
	...
	for (i = 0; i < patch->numfuncs; i++) {
		...
		int32_t jmpoffset =
		    (patch->refabs + func->newrel) - func->oldabs - 5;
...
int _cmd_apply3 (xc_interface_t xch, struct patch3 *patch)
{
	...
	/* Do a list first and make sure patch isn't already applied yet */
	if (find_patch3 (xch, patch->sha1, sizeof (patch->sha1), &info) <
	    0) {
	...
	for (i = 0; i < patch->numdeps; i++) {
		...
		if (find_patch3
			...
			fprintf (stderr,
				 "error: could not search for patches\n");
			return -1;
		}
		if (dep_patch == NULL) {
			char sha1str[SHA_DIGEST_LENGTH * 2 + 1];
			bin2hex (patch->deps[i].sha1,
				 sizeof (patch->deps[i].sha1), sha1str,
				 sizeof (sha1str));
			fprintf (stderr,
 ...
 */
	...
	if (get_xen_version (rxenversion, sizeof (rxenversion)) < 0)
		return -1;
	if (get_xen_compile_date
	    (rxencompiledate, sizeof (rxencompiledate)) < 0)
		return -1;
	...
	if (strcmp (rxenversion, patch.xenversion) != 0 ||
		...
		fprintf (stderr,
			 "error: patch does not match hypervisor build\n");
		return -1;
	}
#else
/* check for QEMU version and sandbox build info */
	...
	if (patch.crowbarabs != 0) {
		fprintf (stderr,
			 "error: cannot handle crowbar style patches\n");
		return -1;
	}
	...
	if (patch.numtables > 0) {
		fprintf (stderr,
			 "error: cannot handle table writes, yet\n");
		return -1;
	}
	...
	char sha1str[SHA_DIGEST_LENGTH * 2 + 1];
	bin2hex (patch.sha1, sizeof (patch.sha1), sha1str,
		 sizeof (sha1str));
...
void print_patch_info3 (struct xenlp_patch_info3 *pi, int last)
{
	...
	for (j = 0; j < MAX_LIST_DEPS; j++) {
		if (memcmp (pi->deps[j].sha1, sha1zero, SHA_DIGEST_LENGTH)
		    == 0)
			break;
		char hex[SHA_DIGEST_LENGTH * 2 + 1];
		bin2hex (pi->deps[j].sha1, SHA_DIGEST_LENGTH, hex,
			 sizeof (hex));
		if (json) {
			printf (" \"dep\": \"%s\",", hex);
		} else
			      ...
			      struct symbols *symbols)
{
	if (locate_symbols_offsets (data, start, symbols) == 0) {
		...
		symbols->offsets =
		    _zalloc (symbols->count * sizeof (uint32_t));

		/* This assumes both the ELF file and host are same endian */
		memcpy (symbols->offsets, buf,
			symbols->count * sizeof (uint32_t));
...
{
	symbols->names =
	    _zalloc (sizeof (symbols->names[0]) * symbols->count);
...
int info_xen_gz (int origfd, const char *filename)
{
	...
	if (extract_symbol_locations (textdata, text->sh_addr, &symbols) <
	    0) {
		fprintf (stderr,
	...
	if (locate_symbols_token_table (textdata, &symbols) < 0) {
		fprintf (stderr,
			 "%s: could not find symbols_token_table\n",
			 filename);
		goto err2;
	}
	...
	uint32_t major_ver =
	    _get_int_func_wrapper (text, textdata, &symbols,
				   "xen_major_version");
	uint32_t minor_ver =
	    _get_int_func_wrapper (text, textdata, &symbols,
				   "xen_minor_version");
	char *extra_ver =
	    _get_string_func_wrapper (text, textdata, &symbols,
				      "xen_extra_version");
		...
		const unsigned char *patch_hash)
{
	...
	if (ret < 0) {
		if (errno == ENOENT) {
			...
			fprintf (stderr,
				 "failed to undo a hypervisor patch: %m\n");
			return -1;
		}
	}
	return 0;
...
#endif
{
	...
	if (caps.flags & XENLP_CAPS_V3) {
		...
		fprintf (stderr,
			 "error: no v3 ABI detected, undo disabled\n");
		ccode = -1;
		goto out;
	}
#ifndef sandbox_port
		...
		case 3:	/* find */
			{
				...
				DMSG ("selected option %s\n",
				      long_options[option_index].name);
				strncpy ((char *) patch_hash, optarg,
...
int main (int argc, char **argv)
{
	...
	if (find_flag > 0) {
		...
		ccode =
		    find_patch (sockfd, sha1, SHA_DIGEST_LENGTH,
				&patch_buf);
		if (ccode == 1) {
			DMSG ("found patch: %s\n", patch_hash);
