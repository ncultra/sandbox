...
int
locate_symbols_offsets (Elf_Data * data, uint32_t start,
...
int
locate_symbols_addresses (Elf_Data * data, struct symbols *symbols)
{
...
int
extract_symbol_locations (Elf_Data * data, uint32_t start,
			  struct symbols *symbols)
{
...
int
locate_symbols_token_table (Elf_Data * data, struct symbols *symbols)
{
...
void
extract_symbols_token_table (Elf_Data * data, struct symbols *symbols)
{
...
int
locate_symbols_names (Elf_Data * data, struct symbols *symbols)
{
...
void
extract_symbols_names (Elf_Data * data, struct symbols *symbols)
{
...
uint32_t
get_symbol (struct symbols *symbols, char *name)
{
  size_t i;
  for (i = 0; i < symbols->count; i++)
    {
...
uint32_t
_get_int_func_wrapper (Elf32_Shdr * text, Elf_Data * textdata,
...
char *
_get_string_func_wrapper (Elf32_Shdr * text, Elf_Data * textdata,
			  struct symbols *symbols, char *name)
{
  uint32_t off = get_symbol (symbols, name);
  unsigned char *buf = textdata->d_buf + off - text->sh_addr;
