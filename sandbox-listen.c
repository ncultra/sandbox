...
int listen_sandbox_sock (struct listen *l)
{
    ...
    len =
	offsetof (struct sockaddr_un, sun_path) + strlen ((char *) l->arg);
...
/* allocate dispatch buffer or not? */
    ...
    *id = SANDBOX_MSG_GET_ID (hbuf);
	...
	ccode =
	    dispatch_apply_response (fd, SANDBOX_MSG_GET_LEN (hbuf), buf);
	break;
	...
	ccode =
	    dispatch_list_response (fd, SANDBOX_MSG_GET_LEN (hbuf), buf);
	break;
