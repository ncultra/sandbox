...
void *listen_thread (void *arg)
{
	...
	do {
		if (l->sock > 0) {
			client_fd =
			    accept_sandbox_sock (l->sock, &client_id);
			if (client_fd < 0) {
...
int dispatch_apply (int fd, int len, void **bufp)
{
	...
	uint32_t ccode = SANDBOX_OK, remaining_bytes =
...
*/
	...
	if (!list_empty (&lp_patch_head3)) {
		...
		rsize =
		    (count * sizeof (list_response)) + sizeof (uint32_t);
...
int dispatch_list_response (int fd, int len, void **bufp)
{
	...
	*bufp = calloc (sizeof (uint8_t),
	...
	if ((ccode =
	     readn (fd, *bufp, remaining_bytes) != remaining_bytes)) {
...
/*  reply = 0 for success, < 0 for not applied or error */
	...
	bin2hex (sha1, sizeof (sha1), sha1_txt_buf,
		 sizeof (sha1_txt_buf) - 1);
...
char *get_sandbox_build_info (int fd)
{
	...
	if (send_rr_buf (fd, SANDBOX_MSG_GET_BLD, SANDBOX_LAST_ARG) ==
	    SANDBOX_OK) {
...
void *sandbox_list_patches (int fd)
{
	...
	if (send_rr_buf (fd, SANDBOX_MSG_LIST, SANDBOX_LAST_ARG) ==
	    SANDBOX_OK) {
		ccode =
		    read_sandbox_message_header (fd, &version, &id, &len,
						 (void **) &listen_buf);
