...
uint8_t *get_sandbox_memory (ptrdiff_t size)
{
	...
	sbox_ptr =
	    (uint8_t *) ALIGN_POINTER ((uintptr_t) sbox_cursor, 0x10);
...
int read_patch_data2 (XEN_GUEST_HANDLE (void) * arg,
...
***/
	...
	if (apply->bloblen) {
		...
		*blob_p = get_sandbox_memory (apply->bloblen);;
		...
		runtime_constant =
		    (uintptr_t) & _start - (uintptr_t) apply->refabs;
	...
	if (apply->numrelocs) {
		...
		memcpy (relocs, arg,
			apply->numrelocs * sizeof (relocs[0]));
		arg =
		    (unsigned char *) arg +
		    (apply->numrelocs * sizeof (relocs[0]));
		uintptr_t off = 0;
...
//            *((uintptr_t)(*blob_p + off)) -= relocrel;
	...
	/* Read writes */
	*writes_p =
	    xzalloc_array (struct xenlp_patch_write, apply->numwrites);
	if (!(*writes_p)) {
		DMSG ("error allocating %d bytes in read_patch_data2\n",
		      apply->numwrites *
	...
	for (i = 0; i < apply->numwrites; i++) {
		...
		if (pw->hvabs < (uintptr_t) & _start
		    || pw->hvabs >= (uintptr_t) & _end) {
...
int xenlp_apply3 (void *arg)
{
	...
	res =
	    read_patch_data2 (arg, (struct xenlp_apply3 *) &apply, &blob,
			      &writes);
	...
	if (apply.numdeps > 0) {
		patch->deps =
		    xzalloc_array (struct xenlp_hash, apply.numdeps);
		if (!patch->deps) {
		...
		if (memcpy
		    (patch->deps, arg,
		     apply.numdeps * sizeof (struct xenlp_hash))) {
